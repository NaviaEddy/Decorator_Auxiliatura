...
void ADecoradorBase::SetJugador(AActor* _Jugador)
{
	...
	if (!Jugador) {
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue, 
			FString::Printf(TEXT("No se realizo la conversion a tipo IJugador")));
	}
}
...
void ADecoradorBase::Empezar()
{
	if (!Jugador) {
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue,
			FString::Printf(TEXT("Estado(): Jugador es null")));
		return;
	}
	else {
...
FString ADecoradorBase::ObtenerAtributos()
{
	if (!Jugador) {
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue,
			FString::Printf(TEXT("ObtenerAtributos(): Jugador es null")));
		return " ";
	}
	else {
...
float ADecoradorBase::Duracion()
{
	if (!Jugador) {
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue,
			FString::Printf(TEXT("Duracion(): Jugador es null")));
		return 0.f;
	}
	else {
